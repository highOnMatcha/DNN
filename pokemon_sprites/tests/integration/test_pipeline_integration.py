...
class TestDataLoaderModelIntegration(unittest.TestCase):
    ...
    def _create_test_image_pair(self, pokemon_id: str):
        ...
        sprite_img = Image.new('RGB', (256, 256), color=(255, 0, 0))  # Red sprite
        artwork_img = Image.new('RGB', (256, 256), color=(0, 0, 255))  # Blue artwork
    ...
    def test_data_loading_to_model_pipeline(self):
        ...
        model = Pix2PixGenerator(input_channels=3, output_channels=3, ngf=32)  # Smaller for testing
        model.eval()
        ...
        print_test_result("test_data_loading_to_model_pipeline", True, 
    ...
    def test_augmentation_model_integration(self):
        ...
        print_test_result("test_augmentation_model_integration", True, 
    ...
    def test_config_driven_pipeline(self):
        ...
        # Verify setup
        self.assertEqual(gen_optimizer.param_groups[0]['lr'], config.learning_rate)
        self.assertEqual(disc_optimizer.param_groups[0]['lr'], config.learning_rate)
        self.assertGreater(len(pairs), 0)
        self.assertIn('train_pairs', dataset_info)
        
        print_test_result("test_config_driven_pipeline", True, 
    ...
    def test_batch_processing_integration(self):
        ...
        for i in range(min(batch_size, len(input_files))):
            ...
            target_tensor = torch.from_numpy(np.array(target_img)).permute(2, 0, 1).float() / 255.0
        ...
        print_test_result("test_batch_processing_integration", True, 
...
class TestTrainingPipelineIntegration(unittest.TestCase):
    ...
    def test_loss_computation_integration(self):
        ...
        print_test_result("test_loss_computation_integration", True, 
    ...
    def test_optimizer_integration(self):
        ...
        real_scores = discriminator(input_images, target_images)
        fake_scores = discriminator(input_images, fake_images.detach())  # Detach to avoid graph issues
        ...
        print_test_result("test_optimizer_integration", True, 
    ...
    def test_model_state_management(self):
        ...
        # Get initial states
        initial_gen_state = generator.state_dict()
        initial_disc_state = discriminator.state_dict()
        ...
        new_generator = Pix2PixGenerator(input_channels=3, output_channels=3, ngf=32)
        new_discriminator = Pix2PixDiscriminator(input_channels=6, ndf=32, n_layers=2)
        ...
        for key in generator.state_dict():
            self.assertTrue(torch.equal(generator.state_dict()[key], loaded_gen_state[key]))
        
        for key in discriminator.state_dict():
            self.assertTrue(torch.equal(discriminator.state_dict()[key], loaded_disc_state[key]))
        
        print_test_result("test_model_state_management", True, 
                         "Model states saved and loaded correctly")

    def test_training_loop_integration(self):
        """Test simplified training loop integration."""
        # Create models
        generator = Pix2PixGenerator(input_channels=3, output_channels=3, ngf=16)  # Smaller for speed
        discriminator = Pix2PixDiscriminator(input_channels=6, ndf=16, n_layers=2)
        ...
        for epoch in range(num_epochs):
            ...
            for input_img, target_img in dataset:
                ...
                real_scores = discriminator(input_batch, target_batch)
                fake_scores = discriminator(input_batch, fake_batch.detach())  # Detach to avoid graph issues
        ...
        print_test_result("test_training_loop_integration", True, 
...
class TestDirectoryStructureIntegration(unittest.TestCase):
    ...
    def test_directory_structure_setup(self):
        ...
        print_test_result("test_directory_structure_setup", True, 
    ...
    def test_checkpoint_management_integration(self):
        ...
        print_test_result("test_checkpoint_management_integration", True, 
    ...
    def test_logging_integration(self):
        ...
        print_test_result("test_logging_integration", True, 
                         f"Log file created with {len(test_messages)} entries")


if __name__ == '__main__':
    print(f"\n{TestColors.BLUE}{TestColors.BOLD}{'='*70}{TestColors.RESET}")
    print(f"{TestColors.BLUE}{TestColors.BOLD}Running Integration Tests for Pokemon Sprite Pipeline{TestColors.RESET}")
    ...
    print(f"\n{TestColors.BLUE}{TestColors.BOLD}{'='*70}{TestColors.RESET}")
    print(f"{TestColors.BLUE}{TestColors.BOLD}Integration Tests Completed{TestColors.RESET}")
    print(f"{TestColors.BLUE}{TestColors.BOLD}{'='*70}{TestColors.RESET}")
