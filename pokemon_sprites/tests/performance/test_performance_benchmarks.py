...
class PerformanceBenchmark:
    ...
    def get_summary(self) -> Dict:
        ...
        total_memory = sum(
            b.get("memory_used", 0) for b in self.benchmarks.values()
        )
...
class TestDataLoadingPerformance(unittest.TestCase):
    ...
    def test_find_valid_pairs_performance(self):
        ...
        for size in test_sizes:
            ...
            metrics = self.benchmark.measure_function(
                find_valid_pairs, sprites_dir, artwork_dir
            )
            ...
            self.assertLess(
                metrics["execution_time"], size * 0.01
            )  # Should be very fast
    ...
    def test_image_processing_performance(self):
        ...
        sprites_dir, artwork_dir = self._create_test_images(
            50, (512, 512)
        )  # Larger images
        ...
        metrics = self.benchmark.measure_function(
            ...
            (256, 256),  # Process subset for speed
        )
    ...
    def test_image_stats_performance(self):
        ...
        metrics = self.benchmark.measure_function(
            calculate_image_stats, image_files
        )
...
class TestAugmentationPerformance(unittest.TestCase):
    ...
    def test_single_augmentation_performance(self):
        ...
        for aug_name, augmentation in augmentations.items():
            for size_name, test_image in self.test_images.items():
                ...
                self.assertLess(
                    avg_time, 0.1
                )  # Less than 100ms per augmentation

        # Print results
        for test_name, avg_time in results.items():
            print_test_result(
                f"augmentation_{test_name}",
                True,
    ...
    def test_augmentation_pipeline_performance(self):
        ...
        for case_name, test_image, batch_size in test_cases:
            ...
            self.assertLess(
                memory_used, 100
            )  # Less than 100MB memory increase

            print_test_result(
                f"pipeline_{case_name}",
                True,
    ...
    def test_augmentation_memory_efficiency(self):
        ...
        for batch_size in batch_sizes:
            ...
            for _ in range(batch_size):
                result = flip(
                    self.test_images["medium"], self.test_images["medium"]
                )
            ...
            if (
                memory_increase > 0
            ):  # Only test if there was measurable memory increase
                self.assertLess(
                    memory_increase, batch_size * expected_memory_per_item
                )

                # Memory should be mostly cleaned up (allow perfect cleanup)
                cleanup_ratio = (
                ...
                self.assertLessEqual(
                    cleanup_ratio, 1.0
                )  # Allow perfect memory cleanup

            print_test_result(
                f"memory_efficiency_batch_{batch_size}",
                True,
...
class TestModelPerformance(unittest.TestCase):
    ...
    def test_model_inference_speed(self):
        ...
        models_to_test = {
            "UNet_small": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=32
            ),
            "UNet_medium": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=64
            ),
            "PatchGAN_small": Pix2PixDiscriminator(
                input_channels=6, ndf=32, n_layers=2
            ),
            "Pix2Pix_small": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=32, n_blocks=4
            ),
        }
    ...
    def test_model_memory_usage(self):
        ...
        for batch_size in batch_sizes:
            ...
            memory_per_sample = (
                memory_used / batch_size if batch_size > 0 else 0
            )
    ...
    def test_training_step_performance(self):
        ...
        print_test_result(
            "test_training_step_performance",
            True,
    ...
    def test_model_parameter_efficiency(self):
        ...
        models_to_test = {
            "UNet_32": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=32
            ),
            "UNet_64": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=64
            ),
            "UNet_128": Pix2PixGenerator(
                input_channels=3, output_channels=3, ngf=128
            ),
            "PatchGAN_32": Pix2PixDiscriminator(
                input_channels=6, ndf=32, n_layers=3
            ),
            "PatchGAN_64": Pix2PixDiscriminator(
                input_channels=6, ndf=64, n_layers=3
            ),
        }
        ...
        for model_name, model in models_to_test.items():
            ...
            # Performance assertions
            max_params = (
                70_000_000  # 70M parameters max (accommodates ngf=128)
            )
            ...
            print_test_result(
                f"model_efficiency_{model_name}",
                True,
...
class TestSystemPerformance(unittest.TestCase):
    ...
    def test_concurrent_operations(self):
        ...
        try:
            ...
            models = [
                Pix2PixGenerator(input_channels=3, output_channels=3, ngf=32)
                for _ in range(3)
            ]

            # Create test inputs
            test_inputs = [torch.randn(1, 3, 64, 64) for _ in range(10)]
            ...
            for i, model in enumerate(models):
                thread = threading.Thread(
                    target=worker, args=(model, test_inputs, i)
                )
            ...
            avg_worker_time = sum(
                r["execution_time"] for r in worker_results
            ) / len(worker_results)
        ...
        for ngf in model_sizes:
            ...
            try:
                ...
                model = Pix2PixGenerator(
                    input_channels=3, output_channels=3, ngf=ngf
                )
                model.eval()
                ...
                if (
                    memory_used > 10.0
                ):  # Only test cleanup if substantial memory was used (10MB+)
                    ...
                    cleanup_efficiency = (
                        1.0  # Skip test for negligible memory operations
                    )
                    efficiency_msg = "N/A (low memory usage)"

                print_test_result(
                    f"memory_pressure_ngf_{ngf}",
                    True,
    ...
    def test_sustained_operation_performance(self):
        ...
        self.assertLess(
            float(cv_time), 15.0
        )  # Very lenient coefficient of variation for variable hardware
        self.assertLess(
            abs(memory_trend), 1.0
        )  # Memory shouldn't trend up significantly

        print_test_result(
            "test_sustained_operation_performance",
            True,
...
if __name__ == "__main__":
    ...
    print(
        f"{TestColors.BLUE}{TestColors.BOLD}Running Performance Tests for Pokemon Sprite Pipeline{TestColors.RESET}"
    )
    ...
    print(f"\n{TestColors.BLUE}{TestColors.BOLD}{'='*70}{TestColors.RESET}")
    print(
        f"{TestColors.BLUE}{TestColors.BOLD}Performance Tests Completed{TestColors.RESET}"
    )
    print(f"{TestColors.BLUE}{TestColors.BOLD}{'='*70}{TestColors.RESET}")
