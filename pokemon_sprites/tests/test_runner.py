...
class PipelineTestRunner:
    ...
    def run_test_file(self, test_file: Path, test_type: str) -> Dict[str, Any]:
        ...
        if spec is None or spec.loader is None:
            ...
            return {'passed': 0, 'failed': 1, 'errors': 0, 'skipped': 0, 'time': 0}
        ...
        try:
            ...
            return {'passed': 0, 'failed': 0, 'errors': 1, 'skipped': 0, 'time': 0}
        ...
        class TestResult(unittest.TextTestResult):
            def __init__(self, stream, descriptions, verbosity):
                ...
                self.test_results = {'passed': 0, 'failed': 0, 'errors': 0, 'skipped': 0}
        ...
        # Print file results
        total_tests = sum(test_results[key] for key in ['passed', 'failed', 'errors', 'skipped'])
        if test_results['failed'] == 0 and test_results['errors'] == 0:
            self.print_success(f"{test_file.name}: {test_results['passed']}/{total_tests} tests passed ({test_time:.2f}s)")
        else:
            self.print_fail(f"{test_file.name}: {test_results['passed']}/{total_tests} tests passed, "
    ...
    def print_final_summary(self):
        ...
        # Calculate totals
        total_passed = sum(self.results[suite]['passed'] for suite in self.results)
        total_failed = sum(self.results[suite]['failed'] for suite in self.results)
        total_errors = sum(self.results[suite]['errors'] for suite in self.results)
        total_skipped = sum(self.results[suite]['skipped'] for suite in self.results)
        ...
        for suite_name, suite_results in self.results.items():
            suite_total = sum(suite_results[key] for key in ['passed', 'failed', 'errors', 'skipped'])
            if suite_total > 0:
                suite_success_rate = (suite_results['passed'] / suite_total) * 100
                status_color = Colors.GREEN if suite_results['failed'] == 0 and suite_results['errors'] == 0 else Colors.RED
                print(f"  {suite_name.capitalize()}: {status_color}{suite_success_rate:.1f}%{Colors.RESET} "
        ...
        if total_failed == 0 and total_errors == 0:
            ...
            self.print_fail(f"TESTS COMPLETED WITH {total_failed} FAILURES AND {total_errors} ERRORS")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Pokemon Sprite Generation Pipeline Tests")
    parser.add_argument(
        '--suite', '-s',
    ...
    runner.print_info("Professional testing framework for ensuring code robustness")
