...
class TestDataFactory:
    ...
    @staticmethod
    def create_test_image(size: tuple = (64, 64), color: str = 'red') -> Image.Image:
        """Create a test image."""
        ...
        def wrapper(*args, **kwargs):
            for module_name in module_names:
                try:
                    ...
                    raise unittest.SkipTest(f"Required module '{module_name}' not available")
            return test_func(*args, **kwargs)
        return wrapper
    return decorator
...
class PerformanceAssertion:
    ...
    @staticmethod
    def assert_memory_usage(actual_memory: float, max_memory: float, test_case):
    ...
    @staticmethod
    def assert_throughput(actual_throughput: float, min_throughput: float, test_case):
