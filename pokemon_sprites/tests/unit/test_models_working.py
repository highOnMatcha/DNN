...
class TestBasicModels(unittest.TestCase):
    ...
    def test_unet_model(self):
        ...
        model = UNet(
            input_channels=3, output_channels=3, features=[32, 64, 128]
        )
        ...
        print_test_result(
            "test_unet_model", True, f"UNet output shape: {output.shape}"
        )
    ...
    def test_pix2pix_discriminator(self):
        ...
        model = Pix2PixDiscriminator(
            input_channels=6, ndf=64
        )  # 6 channels for input+target
        self.assertIsNotNone(model)
    ...
    def test_conv_block(self):
        ...
        print_test_result(
            "test_conv_block", True, f"ConvBlock output shape: {output.shape}"
        )
    ...
    def test_res_block(self):
        ...
        input_tensor = torch.randn(
            self.batch_size, 64, self.height, self.width
        )
        ...
        print_test_result(
            "test_res_block", True, f"ResBlock output shape: {output.shape}"
        )
    ...
    def test_model_parameter_count(self):
        ...
        models_to_test = [
            ...
            (
                "Pix2PixGenerator",
                Pix2PixGenerator(input_channels=3, output_channels=3, ngf=64),
            ),
            (
                "Pix2PixDiscriminator",
                Pix2PixDiscriminator(input_channels=6, ndf=64),
            ),
        ]
...
if __name__ == "__main__":
    print(
        f"{TestColors.BLUE}{TestColors.BOLD}Running Model Tests{TestColors.RESET}\n"
    )
