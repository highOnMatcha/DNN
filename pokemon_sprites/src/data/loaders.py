    ...
    for pokemon_id in pokemon_ids:
        ...
        try:
            if sprite_type == "black-white":
                url = f"https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-v/black-white/{pokemon_id}.png"
...
def find_valid_pairs(sprites_dir: Path, artwork_dir: Path) -> List[Dict]:
    ...
    for sprite_file in sprite_files:
        ...
        elif sprite_file.stem.startswith('pokemon_') and len(sprite_file.stem.split('_')) >= 2:
            ...
            try:
                ...
                with Image.open(sprite_file) as sprite_img:
                    with Image.open(artwork_file) as artwork_img:
                        if (sprite_img.width > 0 and sprite_img.height > 0 and 
                            artwork_img.width > 0 and artwork_img.height > 0):
                            valid_pairs.append({
...
def visualize_dataset_samples(dataset_dir: Path, samples_per_category: int = 3) -> None:
    """
    ...
    # Only include directories that contain image files
    subdirs = []
    for d in dataset_dir.iterdir():
        if d.is_dir():
            image_files = [f for f in d.glob("*") if f.suffix.lower() in ['.png', '.jpg', '.jpeg', '.gif']]
            if image_files:  # Only include directories with images
                subdirs.append(d)
    ...
    for row, subdir in enumerate(subdirs):
        ...
        # Try to find valid images for display
        valid_images = []
        attempts = 0
        max_attempts = min(len(image_files), samples_per_category * 3)  # Try more files to find valid ones
        
        while len(valid_images) < samples_per_category and attempts < max_attempts:
            ...
            try:
                ...
                # Only add if not already in valid_images
        ...
        for col in range(samples_per_category):
            if col < len(valid_images):
                ...
                try:
                    ...
                    # Extract Pokemon ID for better labeling
                    ...
                    axes[row, col].text(0.5, 0.5, f'Display\nError', 
                                       ha='center', va='center', transform=axes[row, col].transAxes)
                    axes[row, col].axis('off')
            else:
                axes[row, col].text(0.5, 0.5, f'No valid\nimages', 
                                   ha='center', va='center', transform=axes[row, col].transAxes)
                axes[row, col].axis('off')
        ...
        try:
            size_mb = sum(f.stat().st_size for f in image_files if f.is_file()) / (1024 * 1024)
        except:
            size_mb = 0
        dataset_stats[subdir.name] = {'files': file_count, 'size_mb': size_mb}
    ...
    for category, stats in dataset_stats.items():
        print(f"- {category.replace('_', ' ').title()}: {stats['files']} files ({stats['size_mb']:.1f} MB)")
...
def analyze_image_characteristics(dataset_info: Dict) -> None:
    ...
    Args:
        dataset_info: Dictionary containing dataset information from create_training_dataset
    """
    ...
    # Print statistics
    print(f"Artwork (Input) Statistics (sample: {input_stats['sample_size']}):")
    print(f"- Brightness: {input_stats['brightness']['mean']:.1f} ± {input_stats['brightness']['std']:.1f}")
    print(f"- Contrast: {input_stats['contrast']['mean']:.1f} ± {input_stats['contrast']['std']:.1f}")
    print(f"- Color complexity: {input_stats['color_complexity']['mean']:.0f} ± {input_stats['color_complexity']['std']:.0f}")
    
    print(f"\nSprites (Target) Statistics (sample: {target_stats['sample_size']}):")
    print(f"- Brightness: {target_stats['brightness']['mean']:.1f} ± {target_stats['brightness']['std']:.1f}")
    print(f"- Contrast: {target_stats['contrast']['mean']:.1f} ± {target_stats['contrast']['std']:.1f}")
    print(f"- Color complexity: {target_stats['color_complexity']['mean']:.0f} ± {target_stats['color_complexity']['std']:.0f}")
    
    # Create comparison visualization
    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    fig.suptitle("Dataset Characteristics Comparison", fontsize=14, fontweight='bold')
    ...
    for idx, (category, title) in enumerate(zip(categories, titles)):
        ...
        axes[idx].bar(['Artwork', 'Sprites'], [input_mean, target_mean], 
...
def visualize_artwork_sprite_pairs(dataset_dir: Path, num_pairs: int = 6) -> int:
    """
        ...
        if '_bw' in sprite_file.stem:
            ...
            if artwork_file.exists():
                try:
                    ...
                    with Image.open(artwork_file) as artwork_img:
                        with Image.open(sprite_file) as sprite_img:
                            if (artwork_img.width > 0 and artwork_img.height > 0 and 
                                sprite_img.width > 0 and sprite_img.height > 0):
                                matched_pairs.append((artwork_file, sprite_file, pokemon_id))
                except Exception:
                    continue
    ...
    # Sample random pairs
    display_pairs = random.sample(matched_pairs, min(num_pairs, len(matched_pairs)))
    
    # Create visualization
    fig, axes = plt.subplots(2, num_pairs, figsize=(3 * num_pairs, 6))
    fig.suptitle("Sugimori Artwork → Black/White Sprite Training Pairs", fontsize=16, fontweight='bold')
    ...
    print(f"- Artwork resolution: Usually 475x475 pixels")
    print(f"- Sprite resolution: Usually 96x96 pixels")
