"""
Centralized logging configuration for the Pokemon sprite generation training
pipeline.
...
class ModelTrainingFilter(logging.Filter):
    ...
    def __init__(
        self, model_name: str = "unknown", experiment_id: str = "unknown"
    ):
        super().__init__()
...
class TrainingProgressLogger:
    ...
    def start_epoch(self, epoch: int, total_epochs: int):
        ...
        self.logger.info(
            f"Starting epoch {epoch + 1}/{total_epochs}",
            extra={
                "extra_fields": {
                    "event": "epoch_start",
                    "epoch": epoch + 1,
    ...
    def log_batch(
        ...
        elapsed = (
            datetime.now() - self.epoch_start_time
        ...
        if total_epochs is not None:
            message = (
                ...
                f"[Batch {batch + 1}/{total_batches}] - {loss_str}"
            )
        else:
            message = (
                f"Epoch {epoch + 1} [Batch {batch + 1}/{total_batches}] - "
                f"{loss_str}"
            )
    ...
    def end_epoch(
        ...
        if total_time:
            log_data["total_training_time_seconds"] = (
                total_time.total_seconds()
            )
            estimated_total = (
                total_time.total_seconds() / (epoch + 1)
            ) * total_epochs
        ...
        message = (
            f"Epoch {epoch + 1}/{total_epochs} completed - Avg {loss_str}"
        )
...
def initialize_project_logging(
    ...
    if experiment_id is None:
        experiment_id = (
            f"{project_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            logger.info(
                ...
                f"({gpu_props.total_memory // 1024**3} GB)"
            )
...
def log_model_summary(
    model, input_shape: tuple, logger_name: str = "model.summary"
):
    """
