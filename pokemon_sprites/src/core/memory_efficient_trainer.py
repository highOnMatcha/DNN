...
This module provides memory-efficient improvements including gradient accumulation,
mixed precision training, advanced loss functions, and efficient attention mechanisms.
"""
...
class MemoryEfficientPokemonTrainer(PokemonSpriteTrainer):
    ...
    def __init__(self, model_config, training_config, wandb_run=None):
        ...
        self.use_mixed_precision = getattr(training_config, 'mixed_precision', True)
        self.gradient_accumulation_steps = getattr(training_config, 'gradient_accumulation_steps', 1)
        self.use_perceptual_loss = getattr(training_config, 'use_perceptual_loss', True)
        self.perceptual_weight = getattr(training_config, 'perceptual_weight', 0.1)
        
        # Anti-blur settings
        self.use_anti_blur_loss = getattr(training_config, 'use_anti_blur_loss', False)
        self.anti_blur_weight = getattr(training_config, 'anti_blur_weight', 10.0)
        self.pixel_art_weight = getattr(training_config, 'pixel_art_weight', 5.0)
        ...
        if self.use_perceptual_loss:
            if self.use_anti_blur_loss:
                ...
                self.logger.info(f"Anti-blur loss enabled with weights: anti_blur={self.anti_blur_weight}, pixel_art={self.pixel_art_weight}")
            else:
                # Use improved perceptual loss only
                self.perceptual_loss = ImprovedPerceptualLoss().to(self.device)
                self.logger.info(f"Improved perceptual loss enabled with weight {self.perceptual_weight}")
    ...
    def _add_attention_modules(self):
        ...
        try:
            if hasattr(self, 'models') and isinstance(self.models, dict) and "generator" in self.models:
                # For Pix2Pix, try to add attention to generator
                generator = self.models["generator"]
                # Add attention as a separate module (will be manually integrated)
    ...
    def _compute_perceptual_loss(self, generated: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_anti_blur_loss and self.combined_loss is not None:
            ...
            return total_loss - 100.0 * F.l1_loss(generated, target)  # Subtract base L1 to avoid double counting
    ...
    def _train_epoch_pix2pix_efficient(self, train_loader, epoch: int) -> Dict[str, float]:
        """Memory-efficient Pix2Pix training with gradient accumulation and mixed precision."""
        total_g_loss = 0.0
        total_d_loss = 0.0
        total_l1_loss = 0.0
        ...
        if not isinstance(self.models, dict):
            raise ValueError("Pix2Pix training requires models to be a dictionary")
        ...
        for batch_idx, (input_imgs, target_imgs) in enumerate(train_loader):
            ...
            with autocast('cuda', enabled=self.use_mixed_precision):
                ...
                with autocast('cuda', enabled=self.use_mixed_precision):
                    ...
                    real_labels = torch.ones(batch_size, 1, output_h, output_w, 
                                           device=self.device, dtype=real_output.dtype)
                    fake_labels = torch.zeros(batch_size, 1, output_h, output_w, 
                                            device=self.device, dtype=real_output.dtype)
            ...
            with autocast('cuda', enabled=self.use_mixed_precision):
                ...
                real_labels_gen = torch.ones(batch_size, 1, output_h, output_w, 
                                           device=self.device, dtype=fake_output.dtype)
                ...
                # Perceptual loss
                g_perceptual_loss = self._compute_perceptual_loss(generated_imgs, target_imgs)
            ...
            if batch_idx % self.training_config.log_frequency == 0:
                self.progress_logger.log_batch(
                    epoch, batch_idx, num_batches,
                    {
                        ...
                        "d_loss": d_loss.item() * self.gradient_accumulation_steps,
...
def create_memory_efficient_trainer(model_config, training_config, wandb_run=None):
    """Factory function to create memory-efficient trainer."""
    return MemoryEfficientPokemonTrainer(model_config, training_config, wandb_run)
