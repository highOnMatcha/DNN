...
class AntiBlurLoss(nn.Module):
    ...
    def _frequency_loss(
        ...
        y, x = torch.meshgrid(
            torch.arange(h, device=generated.device),
            torch.arange(w, device=generated.device),
            indexing="ij",
        )
        distance = torch.sqrt((y - center_h) ** 2 + (x - center_w) ** 2)
        high_pass = (
            distance > min(h, w) * 0.1
        ).float()  # Keep frequencies > 10% of max
...
class ImprovedPerceptualLoss(nn.Module):
    ...
    def forward(
        ...
        input_img = F.interpolate(
            input_img, size=(224, 224), mode="bilinear", align_corners=False
        )
        target_img = F.interpolate(
            target_img, size=(224, 224), mode="bilinear", align_corners=False
        )
