...
class LearningRateFinder:
    ...
    def find_optimal_lr(self, dataloader, start_lr=1e-7, end_lr=1, num_iterations=50, lambda_l1=100.0):
        """Find optimal learning rate using ACTUAL training"""
        
        print(f"LR Range Test: {start_lr:.2e} → {end_lr:.2e} ({num_iterations} iterations)")
        ...
        # Setup optimizers
        gen_optimizer = optim.Adam(self.generator.parameters(), lr=start_lr, betas=(0.5, 0.999))
        disc_optimizer = optim.Adam(self.discriminator.parameters(), lr=start_lr, betas=(0.5, 0.999))
        ...
        for iteration in range(num_iterations):
            try:
                ...
                fake_label = torch.zeros_like(disc_real, device=self.device)
                ...
                fake_label_disc = torch.zeros_like(disc_fake, device=self.device)
                ...
                if total_loss > best_loss * 4:
                    print(f"Early stop at iteration {iteration} - loss diverged")
                    break
                
                if iteration % 10 == 0:
                    print(f"Iter {iteration}: LR {current_lr:.2e}, Loss {total_loss:.4f}")
    ...
    def _analyze_results(self, lrs, losses):
        ...
        print(f"Results: Optimal LR = {optimal_lr:.2e}, Range = {results['min_lr']:.2e} - {results['max_lr']:.2e}")
        return results
...
def find_optimal_learning_rates(config_path, dataset_path=None, num_iterations=30):
    ...
    for model_name, model_config in model_configs.items():
        try:
            ...
            # Run LR finder
            lr_finder = LearningRateFinder(generator, discriminator, str(device))
            results = lr_finder.find_optimal_lr(dataloader, num_iterations=num_iterations)
