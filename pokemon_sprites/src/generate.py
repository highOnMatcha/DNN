...
Pokemon sprites from artwork using the trained image-to-image translation models.
"""
...
from config.settings import get_model_config, ModelConfig
...
class PokemonSpriteGenerator:
    ...
    def __init__(self, model_path: str, device: str = "auto"):
        ...
        self.transform = transforms.Compose([
            transforms.Resize((self.model_config.parameters.get("image_size", 64),
    ...
    def generate_sprite(self, artwork_path: Union[str, Path]) -> Image.Image:
        ...
        generated_sprite = self.inverse_transform(generated_tensor.squeeze(0).cpu())
    ...
    def generate_batch(self, artwork_paths: list, output_dir: str) -> list:
        ...
        for i, artwork_path in enumerate(artwork_paths):
            try:
                ...
                output_path = output_dir / f"{artwork_path.stem}_generated_sprite.png"
                sprite.save(output_path)
                output_paths.append(output_path)
                
                logger.info(f"Generated sprite {i+1}/{len(artwork_paths)}: {output_path}")
                
            except Exception as e:
                logger.error(f"Failed to generate sprite for {artwork_path}: {e}")
                output_paths.append(None)
        
        return output_paths
    
    def create_comparison(self, artwork_path: Union[str, Path], 
        ...
        artwork_resized = artwork.resize((artwork_width, target_height), Image.Resampling.LANCZOS)
        sprite_resized = sprite.resize((sprite_width, target_height), Image.Resampling.NEAREST)  # Pixel art should use nearest neighbor
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Pokemon sprites from artwork")
    ...
    try:
        ...
        if input_path.is_file():
            ...
            if args.comparison:
                comparison_path = output_path / f"{input_path.stem}_comparison.png"
            ...
            if args.comparison:
                for img_file in image_files:
                    comparison_path = output_path / f"{img_file.stem}_comparison.png"
                    generator.create_comparison(img_file, comparison_path)
            else:
                output_paths = generator.generate_batch(image_files, output_path)
                successful = sum(1 for p in output_paths if p is not None)
                logger.info(f"Successfully generated {successful}/{len(image_files)} sprites")
